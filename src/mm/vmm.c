...
__attribute__((no_sanitize("undefined"))) void vmm_switch_pm(pagemap *pm) {
  asm volatile("mov %0, %%cr3" ::"r"((u64)PHYSICAL(pm)) : "memory");
}

__attribute__((no_sanitize("undefined"))) void vmm_init() {
  ...
  vmm_switch_pm(vmm_kernel_pm); /* after running this, when i call dprintf i get
                                   a triple fault!*/
  outb(0xe9, 'A');              /* this doesnt fault though */
  dprintf("vmm: kernel page map located at %lx", (u64)vmm_kernel_pm);
}

__attribute__((no_sanitize("undefined"))) uptr *
vmm_get_next_lvl(uptr *lvl, uptr entry, u64 flags, bool alloc) {
...
__attribute__((no_sanitize("undefined"))) void vmm_map(pagemap *pm, uptr virt,
                                                       uptr phys, u64 flags) {
